...
http.route({
  path: "/clerk-delete-webhook",
  method: "POST",
  handler: httpAction(async (ctx, request) => {
    const webhookSecret = process.env.CLERK_DELETE_WEBHOOK_SECRET;
    if (!webhookSecret) {
      throw new Error("Missing CLERK_DELETE_WEBHOOK_SECRET environment variable");
    }
    ...
    if (eventType === "user.deleted") {
      // delete the user from convex db
      const { id } = evt.data;
